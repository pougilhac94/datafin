...
function affichageSunburst(monfichier, monentete) {
	...
	//ne pas introduire une transition car sa duréee sle remove s'effectuera après création du nouveau svg
	d3.select('#col-centre').selectAll("svg").remove();
	...
	var tableauNiveau = new Array();

	// Tableau des programmes
	var tableauPGM = new Array();
			
	// Total size of all segments; A CALCULER.
	var totalSize = 0; 

	// Liste des différents montants (à faire correspondre à des classes distinctes)
	...
	var arc2 = d3.arc()
		...
		.endAngle(function(d) {
			...
			var ratio = d.data.conso && d.value != 0 ? d.data.conso/d.value : 0;
	...
	var arc1 = d3.arc()
		...
		.endAngle(function(d) { 
			...
			var endAngle = Math.max(0, Math.min(2 * Math.PI, x(d.x1)));
			var ecart = endAngle - startAngle;
			var ratio = d.data.prev && d.value != 0 ? Math.min(d.data.prev/d.value, 1) : 0;		
			var endAngleFinal = (startAngle + ecart * ratio);
			return endAngleFinal;
			})
		.innerRadius(function(d) { return Math.max(0, y(d.y0)) + 0.5 * Math.max(0, y(d.y1)-y(d.y0)); })
	...
	const TEXT_EXPLANATION = 'du total de ce niveau, soit';
	...
	var objectNiv1 = new Object();
	objectNiv1.children = new Array();	
	//tsvHierarchy();
	// Add the mouseleave handler to the bounding circle.
	d3.select("#container").on("mouseleave", mouseleave);

	function click(d) {
		var transitionBase = d3.select('#chart').select('svg').select('g').transition()
	...
	function mouseover(d) {
		...
		d3.select('#chart').select('svg').select('g').transition()
			.delay(300)
			.duration(100)
			.selectAll("path")
...
//			.style("visibility", "hidden");
	...
	function updateBreadcrumbs(nodeArray, percentageString, montant, conso, prev, prevSuppl, ecart, libelle) {
	  ...
	  // Merge enter and update selections; set position for all nodes.
		...
		const COMMENTAIRE_TEXTE_1 = ', une consommation de ';
		const COMMENTAIRE_TEXTE_2 = 'Un supplément prévu de ';
		const COMMENTAIRE_TEXTE_2_SUITE = '. La cible est de ';
		const COMMENTAIRE_TEXTE_3 = 'Ressource manquante : ';
		const TEXTE_VIDE = '';
		const TEXTE_2POINTS = ' : ';
		const TEXTE_SOIT = ' soit ';
		const TEXTE_DISPONIBLE = 'Disponible';
		const TEXTE_BLOCAGE = 'Blocage';

		var texte1 = TEXTE_VIDE,
			texte2 = texte1,
			texte3 = texte1;
		texte1 = libelle + TEXTE_2POINTS + percentageString + TEXTE_SOIT + montant;
		...
		if(togglearcconsoCheck && nodeArray.length>0 && nodeArray[nodeArray.length - 1].data.libelle != TEXTE_DISPONIBLE && nodeArray[nodeArray.length - 1].data.libelle != TEXTE_BLOCAGE){
			// Complément sur la première ligne de commentaire (consommation)
			texte1 += conso? (COMMENTAIRE_TEXTE_1 + conso) : (TEXTE_VIDE);
			// Préparation de la seconde ligne de commentaire (consommation prévisionnelle)
			texte2 = prevSuppl?(COMMENTAIRE_TEXTE_2 + prevSuppl) : (TEXTE_VIDE);
			texte2 += prev?(COMMENTAIRE_TEXTE_2_SUITE + prev) : (TEXTE_VIDE);
			// Préparation de la troisième ligne de commentaire (ccible)
			texte3 = (ecart && ecart > 0)?(COMMENTAIRE_TEXTE_3 + ecart) : (TEXTE_VIDE);
		}
...
//			.style("visibility", "");
	...
	function tsvHierarchy() {
		...
		//});
			// il semble qu'il faille inclure la lecture du second fichier dans la première sinon risque que le premier ne soit pas terminé wquand le second commence
			d3.tsv(fichierJSON).then(function(data) {
				var nodeById = new Object();
				var ind1 = 0;
				var ind2 = 0;
				var ind3 = 0;
				...
				data.forEach(function(d, i) {
					if (!d.niv2) {
						...
						objectNiv1.children[ind1].children = new Array();
						ind2 = 0;
						ind3 = 0;
						++ind1;
					}			
				...
				// Affichage des arcs proportionnels à la ressource (class = ressource)
				d3.select('#chart').select('svg').select('g').selectAll(".ressource")
					.data(partition(root).descendants())
					.enter()
					.append("path")
					.attr("class", "ressource")
					.attr("d", arc0)
					.style("fill", fillColor)
					.style("opacity", 1)
					.on("mouseover", mouseover)
					.on("click", click);	
				// Affichage des arcs proportionnels à la prévision de consommation (class = prev)
				d3.select('#chart').select('svg').select('g').selectAll(".prev")
					.data(partition(root).descendants())
					.enter()
					.append("path")
					.attr("class", "prev")
					...
					.style("opacity", 1)
					.on("click", click);
				// Affichage des arcs proportionnels à la consommation (class = conso)
				d3.select('#chart').select('svg').select('g').selectAll(".conso")
					.data(partition(root).descendants())
					.enter()
					.append("path")
					.attr("class", "conso")
